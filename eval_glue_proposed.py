...
def eval_mode_to_flags(mode: str):
    ...
    if mode not in ["zero", "standard", "metaicl", "proposed", "fedicl"]:
        raise ValueError(f"Unknown mode: {mode}")
    if mode == "zero":
        return {"use_pool": False, "use_db": False}
    if mode in ["standard", "metaicl", "fedicl"]:
...
def evaluate_one_client_one_seed(
    ...
    LABEL_SPACE = build_label_space(global_test)
    LABEL_ID_CACHE = {}
...
def main():
    ...
    ORDERED_TASKS = ["rte", "sst2", "qnli", "mnli", "cola", "mrpc", "qqp"]
    DISP = {
        "rte": "RTE", "sst2": "SST-2", "qnli": "QNLI", "mnli": "MNLI",
        "cola": "CoLA", "mrpc": "MRPC", "qqp": "QQP",
    }
    ...
    for seed in seeds:
        ...
        if args.mode == "fedicl":
            ...
            seed_client_overall[seed][-1] = met["overall_acc"]
            with open(rows_path, "a", encoding="utf-8") as f:
                for r in rows:
                    ...
                    c = -1
                    t = str(r["category"])
                    if t in ORDERED_TASKS:
                        sc_task_correct[s][c][t] += int(r["correct"])
                        sc_task_total[s][c][t] += 1
            print(f"[Seed {seed} | FedICL] overall_acc={met['overall_acc']:.4f}")
    ...
    def fmt_pct(x):  # 0~1 -> "00.00"
        return f"{x*100:.2f}"

    seed_client_mean = {}
    for s in seeds:
        client_accs = list(seed_client_overall[s].values())
        seed_client_mean[s] = float(np.mean(client_accs)) if len(client_accs) > 0 else 0.0
    overall_micro_like_mean, overall_micro_like_std = mean_std([seed_client_mean[s] for s in seeds])

    agg = {
        "seed_client_mean": {str(s): seed_client_mean[s] for s in seeds},
        "overall_mean": overall_micro_like_mean,
        "overall_std": overall_micro_like_std,
    }

    seed_task_acc = defaultdict(dict)  # seed -> task -> acc
    for s in seeds:
    ...
    seed_avg = {s: float(np.mean([seed_task_acc[s][t] for t in ORDERED_TASKS])) for s in seeds}

    avg_mean, avg_std = mean_std([seed_avg[s] for s in seeds])

    task_mean, task_std = {}, {}
    for t in ORDERED_TASKS:
        m, sd = mean_std([seed_task_acc[s][t] for s in seeds])
        task_mean[t], task_std[t] = m, sd

    header = ["Avg"] + [DISP[t] for t in ORDERED_TASKS]
    mean_row = [fmt_pct(avg_mean)] + [fmt_pct(task_mean[t]) for t in ORDERED_TASKS]
    std_row  = [fmt_pct(avg_std)]  + [fmt_pct(task_std[t])  for t in ORDERED_TASKS]

    print("\n" + "#" * 90)
    print(f"FINAL | mode={args.mode} | seeds={seeds}")
    print("#" * 90)

    print(f"[Info] Overall (mean over seeds of client-mean, NOT table Avg): {overall_micro_like_mean*100:.2f} ± {overall_micro_like_std*100:.2f}")

    print(f"[Table Avg] Macro mean over tasks (mean±std over seeds): {avg_mean*100:.2f} ± {avg_std*100:.2f}")

    print("\n" + "=" * 90)
    print("[FINAL GLUE Table-ready Results]")
    print("GLUE\t" + "\t".join(header))
    print("Mean\t" + "\t".join(mean_row))
    print("Std\t"  + "\t".join(std_row))

    table_tsv_path = os.path.join(args.save_dir, "glue_table_mean_std.tsv")
    with open(table_tsv_path, "w", encoding="utf-8") as f:
        f.write("GLUE\t" + "\t".join(header) + "\n")
        f.write("Mean\t" + "\t".join(mean_row) + "\n")
        f.write("Std\t"  + "\t".join(std_row) + "\n")

    table_json_path = os.path.join(args.save_dir, "glue_table_mean_std.json")
    out = {
        "mode": args.mode,
        "seeds": seeds,
        "num_clients": args.num_clients,
        "definition": {
            "task_acc": "accuracy per task",
            "seed_task_acc": "A_{s,t} = mean over clients of a_{s,c,t} within seed s",
            "avg": "Avg_s = macro mean over tasks (unweighted) within seed s; then mean/std over seeds",
            "note": "This Avg matches table-style Avg (not micro accuracy over all problems)."
        },
        "avg": {"mean": avg_mean, "std": avg_std},
        "tasks": {t: {"mean": task_mean[t], "std": task_std[t]} for t in ORDERED_TASKS},
        "seed_task_acc": {str(s): {t: seed_task_acc[s][t] for t in ORDERED_TASKS} for s in seeds},
        "seed_avg": {str(s): seed_avg[s] for s in seeds},
        "overall_clientmean": {  
            "seed_client_mean": {str(s): seed_client_mean[s] for s in seeds},
            "mean": overall_micro_like_mean,
            "std": overall_micro_like_std,
        },
    }
    with open(table_json_path, "w", encoding="utf-8") as f:
        json.dump(out, f, indent=2, ensure_ascii=False)

    latex_path = os.path.join(args.save_dir, "glue_table_row.tex")
    def pm(m, s):
        return f"{m*100:.2f}$_{{\\pm {s*100:.2f}}}$"
    latex_cells = [pm(avg_mean, avg_std)] + [pm(task_mean[t], task_std[t]) for t in ORDERED_TASKS]
    ...
    summary = {
        ...
        "seeds": seeds,
        "seed_client_overall": {str(k): v for k, v in seed_client_overall.items()},
        "aggregate": agg,
        "table_avg_macro": {"mean": avg_mean, "std": avg_std},
        "table_tasks": {t: {"mean": task_mean[t], "std": task_std[t]} for t in ORDERED_TASKS},
